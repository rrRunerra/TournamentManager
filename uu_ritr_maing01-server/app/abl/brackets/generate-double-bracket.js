...
flowchart TD
    Start([Start: generateDoubleBracket]) --> Validate{Valid array & <br/> teams.length >= 2?}
    Validate -- No --> ThrowErr[Throw Error]
    Validate -- Yes --> Init[Initialize upperBracket & lowerBracket arrays]

    subgraph Upper_Bracket_Generation
        Init --> UB_Loop{currentTeams.length > 1?}
        UB_Loop -- Yes --> UB_Pair[Pair teams 2 by 2]
        UB_Pair --> UB_CreateMatch[Create match object <br/> Handle Walkovers/Byes]
        UB_CreateMatch --> UB_Store[Store in upperBracket]
        UB_Store --> UB_Next[Set currentTeams = nextRoundTeams]
        UB_Next --> UB_Loop
        
        UB_Loop -- No --> UB_Connect[Link UB rounds <br/> Set nextMatchId for winners]
    end

    subgraph Lower_Bracket_Generation
        UB_Connect --> LB_Setup[Calculate loserFeeds array <br/> Alternate Feed vs. Merge rounds]
        LB_Setup --> LB_Loop{Iterate through <br/> loserFeeds}
        
        LB_Loop -- Yes --> LB_CheckType{Feed Round or <br/> Merge Round?}
        
        LB_CheckType -- Feed --> LB_Feed[Create LB Matches <br/> Link UB Losers & <br/> Prev LB Winners]
        LB_CheckType -- Merge --> LB_Merge[Create LB Matches <br/> Link Prev LB Winners]
        
        LB_Feed --> LB_NextRound[Store as previousLBMatches]
        LB_Merge --> LB_NextRound
        LB_NextRound --> LB_Loop
    end

    subgraph Finalization
        LB_Loop -- No --> LB_Final[Set Lower Final Name <br/> 3rd/4th Place]
        LB_Final --> UB_Final[Set Upper Final Name <br/> 1st/2nd Place]
    end
    
    UB_Final --> Return([Return bracket object])
