...
const NotificationCard = ({ type = 'success', message, subText, onClose, autoClose = true, duration = 5000 }) => {
    ...
    return (
        <div className={`notification-card ${isVisible ? 'notification-card-visible' : 'notification-card-hidden'}`}>
            <svg className="notification-wave" viewBox="0 0 1440 320" xmlns="http://www.w3.org/2000/svg" style={{ fill: config.waveColor }}>
            ...
            <div className="notification-icon-container" style={{ backgroundColor: config.bgColor }}>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" strokeWidth={0} fill="currentColor" stroke="currentColor" className="notification-icon" style={{ color: config.iconColor }}>
                    {config.icon}
                </svg>
            </div>
            <div className="notification-message-container">
                <p className="notification-message" style={{ color: config.textColor }}>{message}</p>
                {subText && <p className="notification-sub-text">{subText}</p>}
            </div>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 15 15" strokeWidth={0} fill="none" stroke="currentColor" className="notification-cross-icon" onClick={handleClose}>
